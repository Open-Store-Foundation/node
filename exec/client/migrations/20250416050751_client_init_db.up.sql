...
CREATE UNIQUE INDEX idx_asset_address ON obj(address);
...
CREATE INDEX idx_assetlink_sync ON assetlink_sync(asset_address, owner_version);
...
CREATE UNIQUE INDEX idx_proof_validation ON validation_proof(asset_address, owner_version);
...
CREATE UNIQUE INDEX idx_publishing_address_id ON publishing(asset_address, track_id);
...
CREATE UNIQUE INDEX idx_artifact_address_version ON artifact(asset_address, version_code);
...
CREATE INDEX idx_reports_asset_address ON report(asset_address);
...
CREATE TABLE achievement (
    ...
    name VARCHAR(50) NOT NULL,
    value VARCHAR(20),
    asset_id BIGINT NOT NULL REFERENCES obj(id) ON DELETE CASCADE,
    created_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP
);
...
CREATE UNIQUE INDEX idx_reviews_object_user_id ON review(asset_id, user_id);
